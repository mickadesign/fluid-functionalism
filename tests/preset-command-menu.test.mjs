// Command menu preset guards, modeled on preset-combobox: round-trip
...
import {
  ...
  COMMAND_MENU_PRESET_FIELDS,
  DEFAULT_COMMAND_MENU_PRESET,
} from "../lib/preset/command-menu-options.ts";
...
describe("command menu preset codec", () => {
  ...
  it("the three globals fields sit LAST, matching the sidebar's layout", () => {
    const tail = COMMAND_MENU_PRESET_FIELDS.slice(-3).map((f) => [f.key, f.bits]);
    expect(tail).toEqual([
      ["flavor", 3],
      ["shape", 2],
      ["size", 2],
  ...
  it("the all-defaults code decodes to the defaults", () => {
    const res = decodeCommandMenuPreset(COMMAND_MENU_DEFAULT_CODE);
    expect(res.ok).toBe(true);
    expect(res.preset).toEqual(DEFAULT_COMMAND_MENU_PRESET);
  });
  ...
  it("round-trips 300 random presets exactly", () => {
    ...
    for (let i = 0; i < 300; i++) {
      ...
      const res = decodeCommandMenuPreset(encodeCommandMenuPreset(p));
      expect(res.ok).toBe(true);
  ...
  it("rejects garbage without throwing", () => {
    for (const bad of ["", "k", "xz99", "kZ!!!", "ka" + "z".repeat(40)]) {
      const res = decodeCommandMenuPreset(bad);
  ...
  it("golden code for a fixed non-default preset is stable", () => {
    const code = encodeCommandMenuPreset({
      shortcut: "mod+p",
      descriptions: false,
      tabs: true,
      filters: true,
    ...
    const back = decodeCommandMenuPreset(code);
  ...
  it("a version-b code still decodes", () => {
    const back = decodeCommandMenuPreset("kb9J");
    expect(back.ok).toBe(true);
  ...
  // Version "a" codes were published before the footer joined the table;
  // they must keep decoding, with the footer at its default.
  it("a version-a code still decodes", () => {
    const back = decodeCommandMenuPreset("ka7O");
    expect(back.ok).toBe(true);
...
const MATRIX = [
  ...
  { footer: false },
  { footer: false, tabs: true, filters: true },
];

describe("command menu preset install generator", () => {
  for (const [i, partial] of MATRIX.entries()) {
    it(`matrix ${i} typechecks: ${JSON.stringify(partial)}`, () => {
      const diags = typecheckPreset({ ...DEFAULT_COMMAND_MENU_PRESET, ...partial });
  ...
  it("fuzz: 100 random presets parse cleanly", () => {
    const rand = rng(42);
    for (let i = 0; i < 100; i++) {
      const p = { ...DEFAULT_COMMAND_MENU_PRESET, ...randomPreset(rand) };
      for (const file of generateCommandMenuPresetFiles(p)) {
  ...
  it("every registry dep exists as a manifest item (or is 'utils')", () => {
    ...
    for (let i = 0; i < 100; i++) {
      const p = { ...DEFAULT_COMMAND_MENU_PRESET, ...randomPreset(rand) };
      for (const dep of commandMenuPresetRegistryDeps(p)) {
