...
const AskUserQuestions = forwardRef<HTMLDivElement, AskUserQuestionsProps>(
  function AskUserQuestions(
    ...
    // the inverse: snap into two abutting halves, then glide apart. Selected
    // backgrounds use shape.bg, so corners animate around its radius.
    const R = shape.bgRadius;
    const [boundaries, setBoundaries] = useState<Boundary[]>([]);
    const prevRunsRef = useRef<Run[]>([]);
    const tidRef = useRef(0);
    const timersRef = useRef(new Map<number, ReturnType<typeof setTimeout>>());
    const runsSig = selectedGroups
    ...
    for (const run of selectedGroups) {
    ...
    for (const p of prevRunsRef.current) {
      const c = bridgePair(p, selectedGroups);
          ...
          >
            ...
            >
              ...
              {/* Selected-row backgrounds (merged for contiguous selections).
                  ...
                  abutting halves — see the boundary animation logic above. Uses
                  bg-active (overlay-aware) and renders ABOVE the hover indicator
                  so the selected state stays readable when mousing over a row.
                  Corners are driven numerically (around shape.bg's radius) so a
                  single selected row matches its hover. */}
              <AnimatePresence>
                {blocks.map((b) => {
                  const corner = b.delayCorners
                    ? { ...mergeSpring, delay: b.cornerDelay ?? cornerDelay }
                    : mergeSpring;
                  const opacity = b.opacity ?? (isHoveringNonSelected ? 0.8 : 1);
                  return (
                    <motion.div
                      key={b.key}
                      aria-hidden
                      className="absolute pointer-events-none bg-active"
                      initial={
                        b.enterFrom
                          ? {
                              opacity,
